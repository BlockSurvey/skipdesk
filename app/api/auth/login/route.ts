...
/** Proxy login to the worker; on success store the session token as an httpOnly cookie. */
export async function POST(req: Request) {
  ...
  if (!res.ok) return NextResponse.json({ error: data.error ?? 'Login failed' }, { status: res.status })
